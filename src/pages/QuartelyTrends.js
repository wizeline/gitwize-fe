        ...
        <Grid className={classes.gridItem} style={{ justifyContent: 'flex-end' }} item xs={8}>
          <Grid container className={classes.root}>
            {chartBars.map((chartItem) => {
              const data =
                chartItem.fieldName === 'percentageRejectedPR'
                  ? transformChartDataWithValueAbove(
                      responseData[chartItem.fieldName],
                      chartItem,
                      dateFrom,
                      dateTo,
                      customFormatter
                    )
                  : transformChartDataWithValueAbove(responseData[chartItem.fieldName], chartItem, dateFrom, dateTo)
              return (
                <Grid key={chartItem.chartId} className={classes.gridItem} item xs={4}>
                  <Chart
                    ...
                    chartLegendId={chartItem.chartId}
                  />
                </Grid>
              )
            })}
