            ...
            <Grid container className={classes.root}>
              {chartBars.map(chartItem => {
                return (<Grid key={chartItem.chartId} className={classes.gridItem} item xs={4}>
                          <Chart data={transformChartDataWithValueAbove(responseData[chartItem.fieldName], chartItem, getMonthFromDate(dateRange.date_from * 1000), getMonthFromDate(dateRange.date_to * 1000))} chartOptions={buildChartOptionsBasedOnMaxValue(responseData[chartItem.fieldName])} chartBars={chartBars} chartLines={chartLines} chartLegendId={chartItem.chartId}/>
                        </Grid>)
              })}
            </Grid>
        </Grid>
